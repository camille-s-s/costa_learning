function RNN = fitCostaRNNv4(RNNname, ...
...
if rmvOutliers
    ...
    subplot(1, 2, 1), histogram(mean(exp_data, 2)), title('mean target FRs all units w/ outliers')
    outliers = isoutlier(mean(exp_data, 2), 'percentiles', [1 99]);
    exp_data = exp_data(~outliers, :);
    arrayUnit = arrayUnit(~outliers, :);
    subplot(1, 2, 2), histogram(mean(exp_data, 2)), title(['mean target FRs all units minus ', num2str(sum(outliers)), ' outliers'])
...
if plotStatus
    figure, subplot(1,2,1), imagesc(allSpikes), title('non-normed rates'), colorbar, colormap jet, subplot(1,2,2), imagesc(exp_data), colorbar, title('target rates')
...
save([rnnDir 'exp_data' filesep, RNNname, '_exp_data'],  'exp_data', 'fixOnInds', 'allPossTS', 'nTrls', '-v7.3')
...
for iTrl = startTrl : nTrls % - 1 or nSets - 1
    ...
    % these three variables will be saved in full for each file for faster
    % training
    ...
    stdData(iTrl)  = std(reshape(inputs(iTarget, :), length(iTarget) * length(tData), 1));
    ...
    for nRun = 1 : nRunTot
        ...
        for tt = 2 : length(tRNN) % why start from 2?
            ...
            if tLearn >= dtData
                ...
                error = R(1 : nUnits, tt) - inputs(1 : nUnits, iLearn); % if curr_exp_data are  currents, compare JR. if rates, compare R
        ...
        pVar = 1 - ( norm(inputs(iTarget,:) - rModelSample, 'fro' ) / ( sqrt(length(iTarget) * length(tData)) * stdData(iTrl)) ).^2;
        pVars(nRun) = pVar;
        ...
        if plotStatus
            ...
            imagesc(inputs(iTarget, :)); colormap(jet), colorbar;
            ...
            plot(tData,inputs(iTarget(idx), :), 'linewidth', 1.5);
