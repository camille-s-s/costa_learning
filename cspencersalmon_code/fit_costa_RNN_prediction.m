...
use_reservoir           = 0;                    % make replicates so you have more units than targes
nReplicates             = 25;                   % if use_reservoir = true you gotta specify this
use_synthetic_targets   = false;                % instead of real data, fake data
%% output options
plotStatus      = true;
saveMdl         = true;
nBins           = 50;
...
exp_data = exp_data(iPredict, :); % note that iPredict ~= iTarget. iPredict = units whose output we wanna predict, iTarget = units whose synapses we modify in trainiing
% update indexing by region so you can figure out which unit from which region is included (when nPredicted < nUnits)
...
arrayRgns_included = [arrayRgns(:, 1 : 2), linear_inds_included_units];
% reservoir attempt IN TESTING 2022-01-03
if use_reservoir
...
training_inputs = cell(nTrlsTrain, 1); training_targets = cell(nTrlsTrain, 1); training_tData = cell(nTrlsTrain, 1);
trl_num = 1;
...
% TO DO: TEST THIS
% synthetic_training_data = generate_synthetic_data(training_inputs, 1 : nReplicates : nUnits, 1, 0.1, 1/3, smoothWidth, dtData);
testing_inputs = cell(nTrlsTest, 1); testing_targets = cell(nTrlsTest, 1); testing_tData = cell(nTrlsTest, 1);
trl_num = 1;
...
if use_synthetic_targets
    ...
    % make the fakes
    synthetic_data  = generate_synthetic_data(all_data, 1 : nReplicates : nUnits, 1, 0.1, 1/3, smoothWidth, dtData);
    % synthetic_train_data  = generate_synthetic_data(training_data, 1 : nReplicates : nUnits, 1, 0.1, 1/3, smoothWidth, dtData);
    % synthetic_test_data  = generate_synthetic_data(testing_data, 1 : nReplicates : nUnits, 1, 0.1, 1/3, smoothWidth, dtData);
    ...
    % synthetic_train_inputs = arrayfun(@(iTrlNum) synthetic_train_data{iTrlNum}(:, 1 : end - n_pred_steps), 1 : nTrlsTrain, 'un', 0);
    % synthetic_train_targets = arrayfun(@(iTrlNum) synthetic_train_data{iTrlNum}(:, 1 + n_pred_steps : end), 1 : nTrlsTrain, 'un', 0);
    % synthetic_test_inputs = arrayfun(@(iTrlNum) synthetic_test_data{iTrlNum}(:, 1 : end - n_pred_steps), 1 : nTrlsTrain, 'un', 0);
    % synthetic_test_targets = arrayfun(@(iTrlNum) synthetic_test_data{iTrlNum}(:, 1 + n_pred_steps : end), 1 : nTrlsTrain, 'un', 0);
end
...
if trainRNN % TRAIN
    ...
    for iTrlID = train_trl_IDs(start_trl_num : end)
        ...
        if strcmp(ntwk_flavor, 'descriptive')
        ...
        if plotStatus && ismember(trl_num, floor(linspace(1, nTrlsTrain, 5)))
            ...
            progCompFigName = [rnnSubDir, RNNname, '_train_trl', num2str(iTrlID), '_num', num2str(trl_num), '_runs'];
            overallCompFigName = [rnnSubDir, RNNname, '_train_trl', num2str(iTrlID), '_num', num2str(trl_num), '_overall'];
            histName = [rnnSubDir, RNNname, '_train_trl', num2str(iTrlID), '_num', num2str(trl_num), '_FR_hists'];
        end
        ...
        [RNN] = make_RNN_struct(trainRNN, doSmooth, smoothWidth, meanSubtract, doSoftNorm, normByRegion, rmvOutliers, ...
            ...
            dtRNN, dtData, J, J0, [], [], sum_MSE_over_runs, mean_MSE_over_runs, pVars, stdData, iPredict, ntwk_flavor, n_pred_steps);
        if saveMdl
    ...
    for iTrlID = test_trl_IDs(start_trl_num : end)
        ...
        if plotStatus && ismember(trl_num, floor(linspace(1, nTrlsTest, 5))) % plot progress as we tweak parameters
            ...
            text(axs(4), 0.25 * range(get(axs(4), 'xlim')), 0.75 * range(get(axs(4), 'ylim')), paramText, 'fontsize', 14, 'fontweight', 'bold', 'horizontalalignment', 'center')
            set(axs(4), 'xtick', '', 'ytick', '')
            progCompFigName = [rnnSubDir, RNNname, '_test_trl', num2str(iTrlID), '_num', num2str(trl_num), '_runs'];
            overallCompFigName = [rnnSubDir, RNNname, '_test_trl', num2str(iTrlID), '_num', num2str(trl_num), '_overall'];
            histName = [rnnSubDir, RNNname, '_test_trl', num2str(iTrlID), '_num', num2str(trl_num), '_FR_hists'];
        end
        ...
        if plotStatus
            if ismember(trl_num, floor(linspace(1, nTrlsTest, 5)))
                ...
                fAxs = findall(f, 'type', 'axes'); set(fAxs(2), 'ylim', [-0.2 1]);
                print('-dtiff', '-r400', overallCompFigName)
                plot_comparative_FR_hists_each_unit(nBins, nUnits, nPredict, max_count, lower_FR_thresh, upper_FR_thresh, R_test(:, iMdlSample_test), targets, fTitle)
                print('-dtiff', '-r400', histName)
        ...
        RNN = make_RNN_struct(trainRNN, doSmooth, smoothWidth, meanSubtract, doSoftNorm, normByRegion, rmvOutliers, ...
            ...
            dtRNN, dtData, J_test, [], [], [], sum_MSE_over_runs_test, mean_MSE_over_runs_test, pVars_test, stdData_test, iPredict, ntwk_flavor, n_pred_steps);
        if saveMdl
    ...
    if saveMdl
        save([rnnSubDir, RNNname, '_metrics.mat'], ...
